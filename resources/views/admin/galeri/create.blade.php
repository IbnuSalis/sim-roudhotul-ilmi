...
@section('title','Tambah Foto') @section('page-title','Tambah Foto Galeri')
@section('content')
<div class="max-w-xl">
    @if($errors->any())
        ...
        <form action="{{ route('admin.galeri.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="judul" value="{{ old('judul') }}" required
                       ...
                       placeholder="Nama kegiatan / keterangan foto">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Kategori</label>
                <input type="text" name="kategori" value="{{ old('kategori') }}"
                       class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary"
                       placeholder="Wisuda, Outing, Ramadhan, dll">
            </div>
            <div>
                ...
                <div id="preview-box" class="hidden mt-3">
                    <img id="preview-img" class="h-40 rounded-xl border border-gray-200 object-cover">
                </div>
            </div>
            <div>
                ...
                <textarea name="deskripsi" rows="3"
                          class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary"
                          placeholder="Keterangan foto (opsional)">{{ old('deskripsi') }}</textarea>
            </div>
            <div>
